...
def afdm(data) :
   ...
   # Extraire les composants principaux sélectionnés
    x_index = x_component - 1
    y_index = y_component - 1
    x_data = data.iloc[:, x_index]
    y_data = data.iloc[:, y_index]

    # Dessiner le nuage de points
    show_row_labels = st.checkbox('Show Row Labels', value=False)
    row_labels_column = st.selectbox('Row Labels Column', data.columns.tolist())

    chart = famd.plot(data, x_component=int(x_component), y_component=int(y_component),
             show_row_labels=show_row_labels, row_labels_column= row_labels_column)
...
def svd(data) :
    ...
    st.write(f"Données après réduction  de dimension")
    st.write(data_svd)
